...
class rcube_ui
{
    ...
    public static function format_date($date, $format = null, $convert = true)
    {
        ...
        for ($i=0; $i<strlen($format); $i++) {
            ...
            // weekday (short)
                ...
                $out .= self::label(strtolower($date->format('D')));
            }
            // weekday long
            else if ($format[$i] == 'l') {
                $out .= self::label(strtolower($date->format('l')));
            }
            // month name (short)
            else if ($format[$i] == 'M') {
                $out .= self::label(strtolower($date->format('M')));
            }
            // month name (long)
            else if ($format[$i] == 'F') {
                $out .= self::label('long'.strtolower($date->format('M')));
            }
            else if ($format[$i] == 'x') {
    ...
    public static function folder_list($attrib)
    {
        ...
        $hook = $RCMAIL->plugins->exec_hook('render_mailboxlist',
            array('list' => $a_mailboxes, 'delimiter' => $delimiter));
        ...
        if ($type == 'select') {
            ...
            self::render_folder_tree_select($hook['list'], $mbox_name, $attrib['maxlength'], $select, $attrib['realnames']);
            $out = $select->show();
        }
        else {
            $js_mailboxlist = array();
            $out = html::tag('ul', $attrib, self::render_folder_tree_html($hook['list'], $mbox_name, $js_mailboxlist, $attrib), html::$common_attrib);
    ...
    public static function html_editor($mode = '')
    {
        ...
        $RCMAIL->output->add_script("rcmail_editor_init($script)", 'foot');
    }
