...
class html {
    ...
    public $uppertags = false;
    public $upperattribs = false;
    public $upperprops = false;
    ...
    public static function tag($tagname, $attrib = array(), $content = null, $allowed_attrib = null) {
        ...
        $suffix = $attrib['nl'] || ($content && $attrib['nl'] !== false && !in_array($tagname, $inline_tags) || self::$newline) ? "\n" : '';
    ...
    public static function attrib_string($attrib = array(), $allowed = null) {
        ...
        foreach ($attrib as $key => $value) {
            ...
            if (in_array($key, array('checked', 'multiple', 'disabled', 'selected'))) {
                if ($value) {
                    $attrib_arr[] = sprintf('%s="%s"', self::_conv_case($key, 'attrib'), $key);
                }
            } else if ($key=='value') {
                $attrib_arr[] = sprintf('%s="%s"', self::_conv_case($key, 'attrib'), Q($value, 'strict', false));
            } else {
                $attrib_arr[] = sprintf('%s="%s"', self::_conv_case($key, 'attrib'), self::_conv_case(Q($value), 'value'));
            }
        }
     ...
     */
    private static function _conv_case($str, $type='attrib') {
        if ($type == 'tag') {
            return self::$uppertags ? strtoupper($str) : strtolower($str);
        } else if ($type == 'attrib') {
            return self::$upperattribs ? strtoupper($str) : strtolower($str);
        } else if ($type == 'value') {
            return self::$upperprops ? strtoupper($str) : strtolower($str);
        }
    }
}
