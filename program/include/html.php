...
class html
{
    ...
    public static function tag($tagname, $attrib = array(), $content = null, $allowed_attrib = null)
    {
        ...
        if ($content || in_array($tagname, self::$containers))
            return sprintf("<%s%s>%s</%s>%s", $tagname, self::attrib_string($attrib, $allowed_attrib), $content, $tagname, $suffix);
        else
     ...
     * @see html::tag()
     */
    public static function div($attr, $cont)
    {
        if (is_string($attr))
     ...
     * @see html::tag()
     */
    public static function p($attr, $cont)
    {
        if (is_string($attr))
            $attr = array('class' => $attr);
     ...
     * @see html::tag()
     */
    public static function img($attr)
    {
        if (is_string($attr))
            $attr = array('src' => $attr);
    ...
    public static function attrib_string($attrib, $allowed = null)
    {
        ...
        foreach ($attrib as $key => $value) {
            ...
            else    // regular tag attributes
                $attrib_arr[] = sprintf('%s="%s"', self::$lc_tags ? $key : $key, Q($value));
        }
...
class html_inputfield extends html
    ...
    {
        $this->attrib = array('type' => $this->type) + $attrib;
    }
...
class html_hiddenfield extends html_inputfield
{
    ...
    public function show()
    {
      ...
      foreach ($this->fields_arr as $attrib) {
          $out .= html::tag($this->tagname, array('type' => $this->type) + $attrib);
      }
      return $out;
    }
...
class html_textarea extends html
{
     ...
     * @return string HTML output
     */
    function show($cont='', $attrib = null)
    {
        // overwrite object attributes
        if (is_array($attrib))
            $this->attrib = array_merge($this->attrib, $attrib);

        return sprintf("<textarea%s>%s</textarea>", html::attrib_string($this->attrib, array_merge(self::$common_attrib, $this->allowed_attrib)), Q($cont));
    }
}


/**
 * Class to create an HTML drowp-down menu
 *
 * @package HTML
 */
class html_select extends html
{
    ...
    function show($select = array(), $attrib = null)
    {
        ...
        foreach ($this->options as $option) {
            ...
            $this->content .= self::tag('option', $attr, $option['text']);
        }
...
class html_table extends html
{
    ...
    function __construct($attrib = array())
    {
        $this->attrib = $attrib + array('summary' => "", 'border' => 0);
    }
